...
| `scripts/master_script.sh`   | Orchestrates the parallel execution of various scripts, including `run_20.sh` to `run_26.sh`.                                                                 |
| `scripts/measure_time.sh`    | Tracks the time taken by `master_script.sh` and logs it into a time log file (`master_script_time.log`).                                                       |
| `scripts/run_20.sh` to `scripts/run_29.sh` | Executes specific Python-based computations with different configurations and epsilon ranges, logging the results for each range.                               |
